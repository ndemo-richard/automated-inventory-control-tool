...
public class FrmSalesRep extends JInternalFrame{
	...
	public FrmSalesRep(Connection srcCN,JFrame getParentFrame) throws SQLException{
		...
		strSQL = "SELECT * FROM Employee ORDER BY First_name ASC";
				...
				if(total != 0){
					try{
							if(JTSlrTable.getValueAt(JTSlrTable.getSelectedRow(),JTSlrTable.getSelectedColumn()) != null){
								JDialog JDEdit = new frm_add_edit_salesrep(false,JFParentFrame,cnSlr,"SELECT * FROM Employee WHERE EmployeeID = " + JTSlrTable.getValueAt(JTSlrTable.getSelectedRow(),0));
								JDEdit.show();
				...
				if(total != 0){
					try{
							if(JTSlrTable.getValueAt(JTSlrTable.getSelectedRow(),JTSlrTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stSlr.executeQuery("SELECT * FROM Employee WHERE EmployeeID = " + JTSlrTable.getValueAt(JTSlrTable.getSelectedRow(),0));
								if(rsPrint.next()==true){
									...
									RecordToPrint += "                         EMPLOYEE                   R E C O R D                                                        \n";
									RecordToPrint += "                              " + SDFDateFormatter.format(CurrentDate).toString() + "\n\n\n";


									RecordToPrint += "-----------------------------------------------------------------------------------\n\n";

									RecordToPrint += " SalesRep ID: " + rsPrint.getString("EmployeeID") + "                 SalesRep Name: " + rsPrint.getString("Name") + "\n\n";

									RecordToPrint += " Primary Address: " + rsPrint.getString("Email") + "\n";
								;
									RecordToPrint += " City: " + rsPrint.getString("CityTown") + "\n";
									RecordToPrint += " County:: " + rsPrint.getString("county") + "\n";
									RecordToPrint += " Email: " + rsPrint.getString("Email") + "\n";
				...
				if(total != 0){
					try{
						if(JTSlrTable.getValueAt(JTSlrTable.getSelectedRow(),JTSlrTable.getSelectedColumn()) != null){
							...
							if(PromptResult==0){
								stSlr.execute("DELETE * FROM Employee  WHERE EmployeeID = " + JTSlrTable.getValueAt(JTSlrTable.getSelectedRow(),0));
								reloadRecord();
		...
		try{
			...
			if(total != 0){
				...
				while(rsSlr.next()){
					...
					Content[rowNum][0] = "" + rsSlr.getString("EmployeeID");
					Content[rowNum][1] = "" + rsSlr.getString("First_name");
					rowNum++;
				}
			}else{
