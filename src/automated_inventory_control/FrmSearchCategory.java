...
public class FrmSearchCategory extends JDialog{
	...
	public FrmSearchCategory(JFrame OwnerForm){
		super(OwnerForm,true);
	    setTitle("Search Category");

		//Start Initalize variables
		String StrListItem[]={"CategoryIndex","CategoryName","Description"};
		...
		public void actionPerformed(ActionEvent e){
			...
			if(srcObj=="search"){
				if(JTFSearchFor.getText().equals("")){
					...
					FrmCategory.reloadRecord("SELECT * FROM Category WHERE " + JCSearchIn.getSelectedItem().toString().replaceAll(" ", "") + " LIKE '%" + JTFSearchFor.getText() + "%' ORDER BY CategoryName ASC");
					dispose();
				}
