...
public class FrmPurchaseOrder extends JInternalFrame{
	...
	public FrmPurchaseOrder(Connection srcCN,JFrame getParentFrame) throws SQLException{
		//Set the form caption and some properties start
		super("PurchaseOrder Records",false,true,false,true);
		...
		strSQL = "SELECT * FROM PO  ORDER BY Date ASC";
	...
	ActionListener JBActionListener = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			...
			//Add Record
			if(srcObj=="add"){
				JDialog JDAdd = new frm_add_purchaseorder(true,JFParentFrame,cnPrd,"");
				JDAdd.show();
			//Modify Record
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								JDialog JDEdit = new frm_add_purchaseorder(false,JFParentFrame,cnPrd,"SELECT * FROM PO WHERE PONo= " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								JDEdit.show();
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stPrd .executeQuery("SELECT * FROM PO WHERE PONo = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								if(rsPrint.next()==true){
									...
									RecordToPrint += "                       PURCHASE ORDER   R E C O R D\n";
									...
									RecordToPrint += " PONO.: " + rsPrint.getString("PONo") + "                 Date: " + rsPrint.getString("Date") + "\n";
									RecordToPrint += "___________________________________________________________________________________\n";
									RecordToPrint += "___________________________________________________________________________________\n\n";


									RecordToPrint += " ItemNo: " + rsPrint.getString("ItemNo") + "\n";
									RecordToPrint += " SupplierID: " + rsPrint.getString("SupplierID") + "\n";
                                                                                                                                                                                                RecordToPrint += " Shipping Address: " + rsPrint.getString("ShipFromAddress") + "\n";
                                                                                                                                                                                                RecordToPrint += " Shipping County: " + rsPrint.getString("ShipFromCounty") + "\n";
                                                                                                                                                                                                RecordToPrint += " Shipping Carrier: " + rsPrint.getString("ShipViaCarrierName") + "\n";
                                                                                                                                                                                                RecordToPrint += " Shipping Fee: " + rsPrint.getString("ShippingFee") + "\n";
                                                                                                                                                                                                RecordToPrint += " Others: " + rsPrint.getString("Others") + "\n";
                                                                                                                                                                                                RecordToPrint += " Description: " + rsPrint.getString("Description") + "\n";
                                                                                                                                                                                                RecordToPrint += " Qty Purchased: " + rsPrint.getString("QtyPurchased") + "\n";
                                                                                                                                                                                                RecordToPrint += " Unit Cost: " + rsPrint.getString("UnitCost") + "\n";
                                                                                                                                                                                                RecordToPrint += " Discount: " + rsPrint.getString("Discount") + "\n";
				...
				if(total != 0){
					try{
						if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
							...
							if(PromptResult==0){
								stPrd.execute("DELETE * FROM PO WHERE PONo = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								reloadRecord();
	...
	public static  JTable CreateTable(){
		...
		try{
			...
			if(total != 0){
				...
				while(rsPrd.next()){
					Content[rowNum][0] = "" + rsPrd.getString("PONo");
					Content[rowNum][1] = "" + rsPrd.getString("ItemNo");
