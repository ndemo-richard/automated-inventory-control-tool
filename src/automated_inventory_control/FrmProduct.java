...
public class FrmProduct extends JInternalFrame{
	...
	public FrmProduct(Connection srcCN,JFrame getParentFrame) throws SQLException{
		...
		strSQL = "SELECT * FROM Item  ORDER BY Description ASC";
	...
	ActionListener JBActionListener = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			...
			//Add Record
			if(srcObj=="add"){
				JDialog JDAdd = new frm_add_edit_product(true,JFParentFrame,cnPrd,"");
				JDAdd.show();
			//Modify Record
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								JDialog JDEdit = new frm_add_edit_product(false,JFParentFrame,cnPrd,"SELECT * FROM Item  WHERE ItemNo = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								JDEdit.show();
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stPrd .executeQuery("SELECT * FROM Item WHERE ItemNo = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								if(rsPrint.next()==true){
									...
									RecordToPrint += "                         PPRODUCT   R E C O R D\n";
									...
									RecordToPrint += " Product ID: " + rsPrint.getString("ItemNo") + "                  Product Description: " + rsPrint.getString("Description") + "\n";
									RecordToPrint += "___________________________________________________________________________________\n";
									RecordToPrint += "___________________________________________________________________________________\n\n";
				...
				if(total != 0){
					try{
						if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
							...
							if(PromptResult==0){
								stPrd.execute("DELETE * FROM Item WHERE ItemNo = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								reloadRecord();
	...
	public static  JTable CreateTable(){
		...
		try{
			...
			if(total != 0){
				...
				while(rsPrd.next()){
					Content[rowNum][0] = "" + rsPrd.getString("ItemNo");
					Content[rowNum][1] = "" + rsPrd.getString("Description");
					Content[rowNum][2] = "" + rsPrd.getString("SKU");
					rowNum++;
				}
			}else{
