...
public class FrmInvoice extends JInternalFrame{
	...
	public FrmInvoice(Connection srcCN,JFrame getParentFrame) throws SQLException{
		...
		super("Invoices Records",false,true,false,true);
		...
		strSQL = "SELECT * FROM Invoices  ORDER BY InvoiceID ASC";
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stPrd .executeQuery("SELECT * FROM Invoices WHERE InvoiceID = '" + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0).toString()+"'");
								if(rsPrint.next()==true){
									...
									RecordToPrint += "                   INVOICE   R E C O R D\n";
									...
									RecordToPrint += " InvoiceID: " + rsPrint.getString("InvoiceID") + "                 Client: " + rsPrint.getString("Client") + "\n";
									RecordToPrint += "___________________________________________________________________________________\n";
									RecordToPrint += "___________________________________________________________________________________\n\n";


									RecordToPrint += "Invoice Date: " + rsPrint.getString("Invoice_date") + "\n";
                                                                                                                                                                                                RecordToPrint += " Due_date: " + rsPrint.getString("Due_date") + "\n";
									RecordToPrint += " Partial_depo: " + rsPrint.getString("Partial_depo") + "\n";
									RecordToPrint += " Discount: " + rsPrint.getString("Discount") + "\n";
                                                                                                                                                                                                RecordToPrint += " Item: " + rsPrint.getString("Item") + "\n";
                                                                                                                                                                                                RecordToPrint += " Description: " + rsPrint.getString("Description") + "\n";
                                                                                                                                                                                                RecordToPrint += " Unit cost: " + rsPrint.getString("Unit_cost") + "\n";
                                                                                                                                                                                                RecordToPrint += " Quantity: " + rsPrint.getString("Quantity") + "\n";
                                                                                                                                                                                                RecordToPrint += " Line Total: " + rsPrint.getString("Line_total") + "\n";
                                                                                                                                                                                                RecordToPrint += " SubTotal: " + rsPrint.getString("SubTotal") + "\n";
                                                                                                                                                                                                RecordToPrint += " Paid To Date: " + rsPrint.getString("Paid_to_date") + "\n";
                                                                                                                                                                                                RecordToPrint += " Balance_due: " + rsPrint.getString("Balance_due") + "\n";
				...
				if(total != 0){
					try{
						if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
							...
							if(PromptResult==0){
								stPrd.execute("DELETE * FROM Invoices WHERE InvoiceID = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								reloadRecord();
	...
	public static  JTable CreateTable(){
		...
		try{
			...
			if(total != 0){
				...
				while(rsPrd.next()){
					Content[rowNum][0] = "" + rsPrd.getString("InvoiceID");
					Content[rowNum][1] = "" + rsPrd.getString("Invoice_Date");
					Content[rowNum][2] = "" + rsPrd.getString("Item");
					rowNum++;
				}
			}else{
