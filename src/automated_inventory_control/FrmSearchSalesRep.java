...
public class FrmSearchSalesRep extends JDialog{
	...
	public FrmSearchSalesRep(JFrame OwnerForm){
		...
		String StrListItem[]={"EmployeeID","First_name"};
		...
		public void actionPerformed(ActionEvent e){
			...
			if(srcObj=="search"){
				if(JTFSearchFor.getText().equals("")){
					JOptionPane.showMessageDialog(null,"Please enter a text to search.","AIS",JOptionPane.WARNING_MESSAGE);
					JTFSearchFor.requestFocus();
				}else{
					FrmSalesRep.reloadRecord("SELECT * FROM Employee WHERE " + JCSearchIn.getSelectedItem().toString().replaceAll(" ", "") + " LIKE '%" + JTFSearchFor.getText() + "%' ORDER BY First_name ASC");
					dispose();
				}
