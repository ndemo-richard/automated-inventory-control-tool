...
public class FrmCustomer extends JInternalFrame{
	...
	public FrmCustomer(Connection srcCN,JFrame getParentFrame) throws SQLException{
		...
		strSQL = "SELECT * FROM Customer  ORDER BY CompanyName ASC";
				...
				if(total != 0){
					try{
							if(JTCusTable.getValueAt(JTCusTable.getSelectedRow(),JTCusTable.getSelectedColumn()) != null){
								JDialog JDEdit = new frm_add_edit_customer(false,JFParentFrame,cnCus,"SELECT * FROM Customer WHERE CustomerID = " + JTCusTable.getValueAt(JTCusTable.getSelectedRow(),0));
								JDEdit.show();
				...
				if(total != 0){
					try{
							if(JTCusTable.getValueAt(JTCusTable.getSelectedRow(),JTCusTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stCus.executeQuery("SELECT * FROM Customer WHERE CustomerID = " + JTCusTable.getValueAt(JTCusTable.getSelectedRow(),0));
								if(rsPrint.next()==true){
									...
									RecordToPrint += " P.O.box:             " + rsPrint.getString("boxno") + "\n";
				...
				if(total != 0){
					try{
						if(JTCusTable.getValueAt(JTCusTable.getSelectedRow(),JTCusTable.getSelectedColumn()) != null){
							...
							if(PromptResult==0){
								stCus.execute("DELETE * FROM Customer WHERE CustomerID = " + JTCusTable.getValueAt(JTCusTable.getSelectedRow(),0));
								reloadRecord();
	...
	public static  JTable CreateTable(){
		...
		try{
			...
			if(total > 0){
				...
				while(rsCus.next()){
					Content[rowNum][0] = "" + rsCus.getString("CustomerID");
					Content[rowNum][1] = "" + rsCus.getString("CompanyName");
					Content[rowNum][2] = "" + rsCus.getString("ContactName");
					rowNum++;
				}
			}else{
