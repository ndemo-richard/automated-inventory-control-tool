...
public class FrmWarehouse extends JInternalFrame{
	...
	public FrmWarehouse(Connection srcCN,JFrame getParentFrame) throws SQLException{
		...
		strSQL = "SELECT * FROM Warehouse ORDER BY WarehouseName ASC";
				...
				if(total != 0){
					try{
							if(JTWrhTable.getValueAt(JTWrhTable.getSelectedRow(),JTWrhTable.getSelectedColumn()) != null){
								JDialog JDEdit = new frm_add_edit_warehouse(false,JFParentFrame,cnWrh,"SELECT * FROM Warehouse WHERE WarehouseID = " + JTWrhTable.getValueAt(JTWrhTable.getSelectedRow(),0));
								JDEdit.show();
				...
				if(total != 0){
					try{
							if(JTWrhTable.getValueAt(JTWrhTable.getSelectedRow(),JTWrhTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stWrh.executeQuery("SELECT * FROM Warehouse WHERE WarehouseID = " + JTWrhTable.getValueAt(JTWrhTable.getSelectedRow(),0));
	...
	public static  JTable CreateTable(){
		String ColumnHeaderName[] = {
			"Warehouse ID","Warehouse Name","Contact Person"
		};
		try{
			...
			if(total != 0){
				...
				while(rsWrh.next()){
					Content[rowNum][0] = "" + rsWrh.getString("WarehouseID");
					Content[rowNum][1] = "" + rsWrh.getString("WarehouseName");
					Content[rowNum][2] = "" + rsWrh.getString("ContactPerson");
					rowNum++;
				}
			}else{
