...
public class FrmPurchaseReceivable extends JInternalFrame{
	...
	public FrmPurchaseReceivable(Connection srcCN,JFrame getParentFrame) throws SQLException{
		//Set the form caption and some properties start
		super("Purchase Request Records",false,true,false,true);
		...
		strSQL = "SELECT * FROM PR  ORDER BY DateRecieve ASC";
	...
	ActionListener JBActionListener = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			...
			//Add Record
			if(srcObj=="add"){
				JDialog JDAdd = new frm_add_purchasereceivable(true,JFParentFrame,cnPrd,"");
				JDAdd.show();
			//Modify Record
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								JDialog JDEdit = new frm_add_purchasereceivable(false,JFParentFrame,cnPrd,"SELECT * FROM PR  WHERE PRNo= " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								JDEdit.show();
				...
				if(total != 0){
					try{
							if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
								...
								ResultSet rsPrint = stPrd .executeQuery("SELECT * FROM PR WHERE PRNo = '" + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0)+"'");
								if(rsPrint.next()==true){
									...
									RecordToPrint += "                       PURCHASE RECEIVABLE   R E C O R D\n";
									...
									RecordToPrint += " PRNo: " + rsPrint.getString("PRNo") + "                 DATE RECEIVED: " + rsPrint.getString("DateRecieve") + "\n";
									RecordToPrint += "___________________________________________________________________________________\n";
									RecordToPrint += "___________________________________________________________________________________\n\n";


									RecordToPrint += " Supplier ID: " + rsPrint.getString("SupplierID") + "\n";
									RecordToPrint += " Item No: " + rsPrint.getString("ItemNo") + "\n";
									RecordToPrint += " Description: " + rsPrint.getString("Description") + "\n";
                                                                                                                                                                                                RecordToPrint += " QtyRecieve: " + rsPrint.getString("QtyRecieve") + "\n";
                                                                                                                                                                                                RecordToPrint += " UnitCost: " + rsPrint.getString("UnitCost") + "\n";
                                                                                                                                                                                                RecordToPrint += " PONo: " + rsPrint.getString("PONo") + "\n";
                                                                                                                                                                                                RecordToPrint += " DateRecieve: " + rsPrint.getString("DateRecieve") + "\n";
                                                                                                                                                                                                RecordToPrint += " Total amount: " + rsPrint.getString("Totalamount") + "\n";
                                                                                                                                                                                                RecordToPrint += " Approved by: " + rsPrint.getString("ApprovedBy") + "\n";
				...
				if(total != 0){
					try{
						if(JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),JTPrdTable.getSelectedColumn()) != null){
							...
							if(PromptResult==0){
								stPrd.execute("DELETE * FROM PR WHERE PRNo = " + JTPrdTable.getValueAt(JTPrdTable.getSelectedRow(),0));
								reloadRecord();
	...
	public static  JTable CreateTable(){
		...
		try{
			...
			if(total != 0){
				...
				while(rsPrd.next()){
					Content[rowNum][0] = "" + rsPrd.getString("PRNo");
					Content[rowNum][1] = "" + rsPrd.getString("DateRecieve");
